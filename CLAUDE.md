...
- **Verify before claiming "new".** Before any spec or plan calls something
  "new", "add", or proposes a refactor:
  - `grep -rn` for the symbol/file/table/route. If it exists, the spec
    describes a *modification*, not an addition. Fix the framing.
  - `Read` the relevant file(s) end-to-end. Skim ≠ read. Memory ≠ code.
  - For any subsystem worked on within the last 60 days, audit it again —
    the codebase moved.
  - Specs in `docs/superpowers/specs/` MUST include a §0 "Codebase
    baseline" section filled in by grep/Read (template:
    `docs/superpowers/SPEC_TEMPLATE.md`). No §0 = spec not commit-ready.
  - **Start the §0 baseline from `docs/codebase/CODEBASE_MAP.md`** — the
    grep-able capability register + per-subsystem docs (generated by the
    2026-06-05 full-codebase sweep). Check the "Already Exists — Don't
    Rebuild" table there before calling anything "new". Then still verify
    against live code (the map is a snapshot, not the source of truth).
  - This rule exists because the AI Security Slice 1 spec was drafted
    2026-06-04 claiming we'd "add NIST AI RMF + OWASP LLM Top 10 as new
    framework packs" when both had been live for weeks (8 AI-family packs
    + 13 mapping rules already in `scanner_core/ai_framework_registry.json`).
    Corrected by commit `225e4d8`. The rule is the durable fix.
