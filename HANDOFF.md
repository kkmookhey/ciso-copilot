...
- **AI route changes require full deploy.** `cdk deploy CisoCopilotAi --hotswap` works for Lambda code/env-only changes. It does NOT fire the `AwsCustomResource` that creates the deployment + re-points the stage, so any route addition/removal/rename needs `cdk deploy CisoCopilotAi` (no `--hotswap`).
...
- **Authorizer is imported as an `IAuthorizer` object literal**, not via a CDK factory (CDK v2 doesn't expose `fromAttributes` for Authorizer). The literal references `CisoCopilotApi-CognitoAuthorizerId` and adds zero CFN resources — see `platform/lib/ai-stack.ts` for the pattern when adding new authed AI routes.
- **Single CR does atomic deploy + stage update.** `CreateDeploymentAndPointStage` calls `apigateway:createDeployment` with `stageName='v1'` — the API call atomically creates a fresh deployment snapshot (at CR execution time, AFTER all Methods are live) AND points the stage at it. We do NOT use a CFN-managed `apigw.Deployment` because CFN's CreateDeployment misses cross-stack Methods (verified bug at deploy time 2026-06-10; see commit `a61df00`).
- **REDEPLOY_TRIGGER lever in `ai-stack.ts`.** Constant string near the top of `ai-stack.ts`'s CR block. Bump it when you intentionally need to force a fresh deployment without changing any Lambda ARN (e.g., to recover from a state where AI routes aren't serving). The `description` field of `createDeployment` embeds it; when it changes, the CR fires.
- **Both stacks together must stay under 500 resources each.** CisoCopilotApi: ~494. CisoCopilotAi: ~10 after this work. Headroom for Sub-slice 1.4+1.5: ~485 in CisoCopilotAi, ~6 in CisoCopilotApi. Watch the latter — non-AI features will eat that fast.

**Known limitations:**

- **Stack teardown footgun.** If `CisoCopilotAi` is ever destroyed (full `cdk destroy CisoCopilotAi` or stack reprovision), the CR has no `onDelete` handler, so the `v1` stage is left pointing at the now-orphaned deployment ID. Every AI-route call would 404 (and other routes might 5xx) until the stage is manually re-pointed. Mitigation: before any teardown, run `aws apigateway create-deployment --rest-api-id <RestApiId> --stage-name v1 --description "post-CisoCopilotAi-teardown"` to publish a fresh deployment of just CisoCopilotApi's view, then it's safe to destroy `CisoCopilotAi`.
- **CisoCopilotApi-only deploys can break AI routes.** If `CisoCopilotApi` is deployed (e.g., to add a non-AI route) WITHOUT redeploying `CisoCopilotAi` afterwards, its own Deployment may rotate and the new snapshot might not include the cross-stack `/v1/ai/_health` (and future Workspace) routes. The `aiStackExtensionVersion: 'v1'` pin guards against accidental rotation, but if you change `api-stack.ts` in any way that affects its own methods, the pin can't help. Recovery: `cdk deploy CisoCopilotAi --require-approval never` re-fires the CR, which publishes a fresh deployment that includes both stacks' methods. **Convention: any CisoCopilotApi deploy should be followed by a CisoCopilotAi deploy.**
