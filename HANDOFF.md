...
>
> Last updated: 2026-05-21 (AWS scanner uplift Slice 0 shipped — Fargate
> migration + coverage scorecard).

## 🚀 AWS Scanner Uplift — Slice 0 shipped (2026-05-21)

Roadmap major item #1 ("Scanner comprehensiveness uplift") has begun.
Spec: `docs/superpowers/specs/2026-05-20-aws-scanner-uplift-design.md`.
Slice 0 plan: `docs/superpowers/plans/2026-05-20-aws-scanner-uplift-slice-0.md`.
**Branch `feat/aws-scanner-uplift` — pushed + PR open, NOT yet merged to main.**

**The plan: tiered scanning.** Quick / Medium / Deep tiers. Quick = auto
on AWS connect (fast first-login result). Medium = scheduled, full
posture breadth. Deep = paid, on-demand (Lambda code review, image vuln
scanning, priv-esc enumeration). All on uniform ECS Fargate. Full
rationale + the 7-slice phasing in the spec.

**What Slice 0 shipped (foundation only — the coverage engine is Slice 1):**

- **Scanner runs on ECS Fargate, not Lambda.** New `ciso-copilot-scan`
  ECS cluster + `ciso-copilot-aws-scan` Fargate task def (in `ScanStack`).
  The container runs `python run.py` — a new Fargate entrypoint that
  reads scan params from env vars — instead of the Lambda handler. The
  uplifted scan will exceed Lambda's 15-min ceiling; Fargate has none.
- **`onboarding_aws_complete` now triggers `ecs:RunTask`,** not a Lambda
  invoke. The initial scan starts a Fargate task at `scan_tier=quick`.
- **`scans.tier` column** — migration `009_scan_tier.sql` (applied to dev
  Aurora). `quick` | `medium` | `deep`, default `quick`.
- **Coverage scorecard** — `docs/coverage/aws-scorecard.md` + `.json`,
  generated by `platform/lambda/shasta_runner/scripts/gen_scorecard.py`,
  anchored to CIS AWS / FSBP / PCI DSS v4 / NIST 800-53 Rev5 catalogs
  (`app/coverage/benchmarks/`). Baseline: **CIS 74.2%, FSBP 20.5%,
  PCI 9.6%, NIST 10.8%** (Shasta's existing checks only — Slices 1+ raise
  it). A unit test asserts the committed scorecard stays current.
- The legacy `shasta-runner` **Lambda is kept** — still the manual
  re-scan path (`POST /scans/trigger`). Slice 1+ retires it.

**Verified:** a real Fargate scan ran end-to-end (task exit 0, `scan
complete: 511 entities / 785 findings`, `scans.tier=quick`). 40 scanner
unit tests pass — run them with `cd platform/lambda/shasta_runner &&
./.venv/bin/python -m pytest app/tests/` (a `.venv` was added there; it's
gitignored).

**Gotcha:** EC2 rejects non-ASCII in a security-group `description` — an
em-dash 400'd the first `cdk deploy`. Keep SG descriptions plain ASCII.

**Next: Slice 1** — the in-repo posture check engine + first new AWS
services + tier filtering. Needs its own plan.
