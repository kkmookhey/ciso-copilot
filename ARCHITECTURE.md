...
- API Gateway stages don't auto-redeploy when routes are added from a different stack. Mitigated by an `AwsCustomResource` in `CisoCopilotAi` that calls `apigateway:createDeployment` with `stageName='v1'` on every deploy — one API call atomically creates a fresh deployment snapshot (capturing the live API state at that moment, including cross-stack methods) AND points the stage at it.
- We do NOT use a CFN-managed `apigw.Deployment` resource. At deploy time we discovered a reproducible bug where CFN's CreateDeployment call did NOT include cross-stack Methods in its snapshot — even with explicit `addDependency` from the Deployment to the Method. The same API call made manually via aws-cli works correctly. The CR-managed approach sidesteps the CFN-API-Gateway timing window. See commit `a61df00` for the diagnosis.
- `CisoCopilotApi`-only redeploys would otherwise overwrite the served deployment and drop AI routes. Mitigated partially by a `latestDeployment?.addToLogicalId({ aiStackExtensionVersion: 'v1' })` pin in `CisoCopilotApi` that keeps its deployment logicalId stable across most non-AI changes. The remaining risk: if `CisoCopilotApi`'s own methods change, its deployment WILL rotate, and the new snapshot may not include cross-stack AI routes. Recovery: re-deploy `CisoCopilotAi` after any `CisoCopilotApi` deploy that touches methods. Convention is documented in HANDOFF.md.
...
- API Gateway requires unique path segments under a parent — a second stack calling `api.root.addResource('ai')` will 409 with `AlreadyExists`. CisoCopilotApi exports `AiResourceId` (the `/ai` resource's id); CisoCopilotAi imports it via `apigw.Resource.fromResourceAttributes()` and attaches its `_health` (and future Workspace) routes as children of the imported resource.
