...
final class AuthManager: NSObject, ASWebAuthenticationPresentationContextProviding {
    ...
    private enum CognitoConfig {
        ...
        static let clientId = "2r71e13kahf79bvb9stuehm3il"
    ...
    /// Sign-in using a discovered authorize URL (per-tenant Microsoft IdP or
    /// the generic Google IdP). The URL is generated by /auth/discover-tenant
    /// and already carries the right `identity_provider` hint so the Cognito
    /// Hosted UI skips the picker.
    func signIn(authorizeURL: URL) async {
        state = .signingIn
        do {
            let code = try await runWebAuthFlow(authorizeURL: authorizeURL)
            print("[Auth] OAuth code received (len=\(code.count))")
            try await exchangeCodeForTokens(code: code)
            print("[Auth] Tokens exchanged + persisted")
            state = .signedIn
        } catch {
    ...
    private func runWebAuthFlow(authorizeURL: URL) async throws -> String {
        try await withCheckedThrowingContinuation { (cont: CheckedContinuation<String, Error>) in
            let session = ASWebAuthenticationSession(
                ...
                if let error = error {
                    print("[Auth] ASWebAuth error: \(error)")
                    cont.resume(throwing: error); return
                }
                guard let callbackURL = callbackURL else {
                    print("[Auth] no callbackURL")
                    cont.resume(throwing: AuthError.missingCode); return
                }
                print("[Auth] callbackURL: \(callbackURL.absoluteString)")
                let comps = URLComponents(url: callbackURL, resolvingAgainstBaseURL: false)
                let items = comps?.queryItems ?? []

                // Surface Cognito/IdP error responses instead of silently dropping them.
                if let err  = items.first(where: { $0.name == "error" })?.value {
                    let desc = items.first(where: { $0.name == "error_description" })?.value ?? ""
                    cont.resume(throwing: AuthError.idpError(err, desc))
                    return
                }
    ...
    enum AuthError: LocalizedError {
        ...
        var errorDescription: String? {
            ...
            case .idpError(let code, let desc):
                return "IdP error: \(code)\(desc.isEmpty ? "" : " — \(desc)")"
